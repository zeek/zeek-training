...
{
	Input::add_table([
	    $source=smtp_indicator_feed,
	    $name="smtp_malicious_indicators",
	    $idx=smtp_MaliciousIdx,
	    $val=smtp_maliciousVal,
	    $destination=smtp_malicious_indicators,
	    $mode=Input::REREAD]);
}
...
event SMTP::log_smtp(rec: SMTP::Info)
{
	...
	if ( rec?$rcptto ) {
		for ( rcptto in rec$rcptto ) {
			...
			if ( rcptto in smtp_malicious_indicators ) {
				NOTICE([
				    $note=Malicious_rcptto,
				    $msg=fmt("Malicious rectto :: %s, %s", smtp_malicious_indicators[rcptto], rcptto),
				    $email_body_sections = vector(fmt("MessageID: %s", rec$msg_id)),
				    $conn=c,
				    $sub=rcptto,
				    $identifier=cat(rcptto),
				    $suppress_for=1 mins]);
			}
		}
	}
	...
	if ( rec?$to ) {
		for ( to in rec$to ) {
			...
			if ( to in smtp_malicious_indicators ) {
				NOTICE([
				    $note=Malicious_Mailto,
				    $msg=fmt("Malicious to:: %s, %s", smtp_malicious_indicators[to], to),
				    $email_body_sections = vector(fmt("MessageID: %s", rec$msg_id)),
				    $conn=c,
				    $sub=to,
				    $identifier=cat(to),
	...
	if ( rec?$mailfrom ) {
		...
		if ( mailfrom in smtp_malicious_indicators ) {
			NOTICE([
			    $note=Malicious_Mailfrom,
			    $msg=fmt("Malicious MailFrom :: %s, %s", smtp_malicious_indicators[mailfrom], rec$mailfrom),
			    $email_body_sections = vector(fmt("MessageID: %s", rec$msg_id)),
			    $conn=c,
			    $sub=rec$mailfrom,
			    $identifier=cat(rec$mailfrom),
			    $suppress_for=1 mins]);
		}
	}
	...
	if ( rec?$from ) {
		if ( rec$from in smtp_malicious_indicators ) {
			NOTICE([
			    $note=Malicious_from,
			    $msg=fmt("Malicious Sender :: %s, %s", smtp_malicious_indicators[rec$from], rec$from),
			    $email_body_sections = vector(fmt("MessageID: %s", rec$msg_id)),
			    $conn=c,
			    $sub=rec$from,
			    $identifier=cat(rec$from),
			    $suppress_for=1 mins]);
		}
	}
	...
	if ( rec?$reply_to && rec$reply_to in smtp_malicious_indicators ) {
		NOTICE([
		    $note=Malicious_reply_to,
		    $msg=fmt("Malicious reply_to:: %s, %s", smtp_malicious_indicators[rec$reply_to], rec$reply_to),
		    $email_body_sections = vector(fmt("MessageID: %s", rec$msg_id)),
		    $conn=c,
		    $sub=rec$reply_to,
		    $identifier=cat(rec$reply_to),
		    $suppress_for=1 mins]);
	}

	if ( rec?$subject && rec$subject in smtp_malicious_indicators ) {
		NOTICE([
		    $note=Malicious_subject,
		    $msg=fmt("Malicious Subject:: %s, %s", smtp_malicious_indicators[rec$subject], rec$subject),
		    $email_body_sections = vector(fmt("MessageID: %s", rec$msg_id)),
		    $conn=c,
		    $sub=rec$subject,
		    $identifier=cat(rec$subject),
		    $suppress_for=1 mins]);
	}
	...
	if ( rec?$path ) {
		for ( path in rec$path ) {
			...
			if ( ip in smtp_malicious_indicators ) {
				NOTICE([
				    $note=Malicious_Path,
				    $msg=fmt("Blacklisted IP in smtp relay Path: %s %s", smtp_malicious_indicators[ip], ip),
		    	 	    $email_body_sections = vector(fmt("MessageID: %s", rec$msg_id)),
				    $conn=c,
				    $sub=ip,
				    $identifier=cat(ip),
...
event SMTPurl::process_smtp_urls(c: connection, url: string)
{
	if ( url in smtp_malicious_indicators )
		NOTICE([
		    $note=Malicious_URL,
		    $msg=fmt("Malicious URL: %s, %s", url, c$smtp),
		    $email_body_sections = vector(fmt("MessageID: %s", c$smtp$msg_id)),
		    $conn=c,
		    $sub=url,
		    $identifier=cat(url),
		    $suppress_for=1 mins]);
