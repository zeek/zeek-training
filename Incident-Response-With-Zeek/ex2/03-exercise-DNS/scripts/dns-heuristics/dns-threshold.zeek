...
export {
	redef enum Notice::Type += { Threshold, Spike, };

	const dns_threshold: vector of count = { 100000, 200000, 300000, 400000,
	    500000, 800000, 1000000,  } &redef;

	const dns_spike_counters: vector of count = { 100,  } &redef;
	...
	global dns_spike_ip_counter: table[addr] of DNS_SpikeThresholdRecord
	    &create_expire=1sec &expire_func=flush_dns_spike_ip_counter &redef;

	global dns_spike_idx: table[addr] of count &default=0 &create_expire=100msec
	    &redef;

	global dns_spike_idx_2: table[addr] of count &default=0 &create_expire=3min
	    &redef;
	...
	redef DNS::dns_servers += { 131.243.121.19, 131.243.5.2, 128.3.34.186,
	    131.243.64.2, 131.243.64.3, [2620:83:8000:140::3], 
	    [2620:83:8000:140::2], 131.243.5.1, 131.243.84.10 };
	...
	global dns_ip_thresholds: table[addr] of count &create_expire=1days &redef;

	global dns_ip_threshold_idx: table[addr] of count &default=0 &write_expire=1day
	    &redef;
...
event DNS::log_dns(rec: DNS::Info)
	...
	if ( rec?$query && request_ip !in dns_servers )
		...
		if ( ( dns_spike_ip_counter[request_ip]$cur_time -
		    dns_spike_ip_counter[request_ip]$first_seen_time ) <= 1sec )
			{
			...
			if ( n >= Spike_Threshold )
				...
				msg = fmt("IP[%s] has done %s (First:%s, cur: %s, diff: %s) look ups: %d",
				    ...
				    dns_spike_ip_counter[request_ip]$cur_time - dns_spike_ip_counter[request_ip]$first_seen_time,
				    n);
				...
				if ( |dns_spike_idx_2[request_ip]| >= DNS::Spike_Rolling_Threshold )
					...
					if ( time_diff < 60sec )
						{
						NOTICE([ $note=DNS::Spike, $src=request_ip, $msg=msg, $identifier=cat(
						    request_ip) ]);
		...
		if ( check_thresh )
			...
			NOTICE([ $note=DNS::Threshold, $src=request_ip, $msg=msg, $identifier=cat(
			    request_ip), $suppress_for=1min ]);
			}
		} # if dns
	} # end of event
