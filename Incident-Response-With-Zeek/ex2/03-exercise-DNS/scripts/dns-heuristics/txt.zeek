...
export {
	redef enum Notice::Type += { TxtThreshold, TxtSpike, VersionBind, };
	...
	global txt_queries: table[addr] of txt_stats = table() &create_expire=1day;

	const txt_threshold: vector of count = { 100, 250, 500, 1000, 5000, 10000,
	    100000, 200000, 300000, 400000, 500000, 800000, 1000000,  } &redef;

	global txt_threshold_idx: table[addr] of count &default=0 &write_expire=1day
	    &redef;

	const query_threshold: vector of count = { 100, 250, 500, 1000, 10000, 50000,
	    100000, 1000000,  } &redef;

	global query_threshold_idx: table[addr] of count &default=0 &write_expire=1day
	    &redef;
...
event DNS::aggregate_stats(request_ip: addr, query: string, qtype_name: string,
	...
	if ( check_thresh )
		...
		NOTICE([ $note=DNS::TxtThreshold, $src=request_ip, $n=n, $msg=msg,
	...
	if ( check_thresh )
		...
		msg = fmt("IP[%s] has done %s look ups: %s", request_ip, n,
		    txt_queries[request_ip]);
		NOTICE([ $note=DNS::VersionBind, $src=request_ip, $n=q, $msg=msg,
		    $identifier=cat(request_ip), $suppress_for=1min ]);
		}
	}
