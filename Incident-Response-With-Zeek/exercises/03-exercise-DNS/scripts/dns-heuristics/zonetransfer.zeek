...
export { 
	...
	global axfr_queries: table[addr] of count &create_expire = 1 day ;
        ...
        const axfr_threshold: vector of count = {
                1, 2, 5, 10, 25, 1000, 10000, 100000, 
        } &redef;

        global axfr_threshold_idx: table[addr] of count
...
event DNS::aggregate_stats(request_ip: addr, query: string, qtype_name: string, rcode_name: string)&priority=-10
{
	...
	axfr_queries[request_ip] += 1 ; 
        ...
        local check_thresh = check_threshold(axfr_threshold, axfr_threshold_idx, request_ip, n);


        if (check_thresh)
        {
                local msg = fmt ("IP[%s] tried %s failed zonetransfer: %s", request_ip, n, axfr_queries[request_ip]);
                NOTICE([$note=DNS::ZoneTransfer, $src=request_ip, $n=n, $msg=msg, $identifier=cat(request_ip), $suppress_for=1 min]);
        }
...
event DNS::log_dns(rec: DNS::Info)
{
	...
	@if ( Cluster::is_enabled())
		Cluster::publish_hrw(Cluster::proxy_pool, request_ip, DNS::aggregate_stats, request_ip, rec$query, rcode_name) ; 
	@else 
