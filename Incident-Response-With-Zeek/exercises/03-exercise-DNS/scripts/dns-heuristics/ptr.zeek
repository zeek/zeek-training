...
export {
	redef enum Notice::Type += { PTRThreshold, PTRSpike, PTRdone, };

	type ptr_stats: record {
		#ptr_counts: opaque of cardinality &default=hll_cardinality_init(0.1, 0.999);
	...
	global ptr_queries: table[addr] of ptr_stats = table() &create_expire=2day;

	const ptr_threshold: vector of count = { 500, 1000, 5000, 10000, 20000, 50000,
	    100000, 200000, 300000, 400000, 500000, 800000, 1000000,  } &redef;

	global ptr_threshold_idx: table[addr] of count &default=0 &write_expire=1day
	    &redef;
}
...
event DNS::aggregate_stats(request_ip: addr, query: string, qtype_name: string,
	...
	local check_thresh = check_threshold(ptr_threshold, ptr_threshold_idx,
	    request_ip, n);

	if ( check_thresh )
		{
		local msg = fmt("IP[%s] has done %s look ups: %s", request_ip, n,
		    ptr_queries[request_ip]);
		NOTICE([ $note=DNS::PTRThreshold, $src=request_ip, $msg=msg ]);
		}
	}
...
event zeek_done()
	...
	for ( request_ip in ptr_queries )
		{
		local sum = ptr_queries[request_ip]$noerror + ptr_queries[request_ip]$nxdomain +
		    ptr_queries[request_ip]$refused +
		    ptr_queries[request_ip]$unknown;
		local out = fmt("ptr_queries: %s, %s, %s, sum: %s", request_ip,
		    ptr_queries[request_ip], ptr_queries[request_ip]$ptr_counts,
		    sum);

		NOTICE([ $note=DNS::PTRdone, $src=request_ip, $msg=out ]);
