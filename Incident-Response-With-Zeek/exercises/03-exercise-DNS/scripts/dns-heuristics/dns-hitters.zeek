...
export { 
	...
	const host_threshold: vector of count = {
        ...
        global host_threshold_idx: table[addr] of count
                        &default=0 &write_expire = 1 day ; 
	
	global DNS::summary:event(src: addr, resp: addr, query: string, qtype_name: string, rcode_name: string); 
	...
	global expire_heavy_hitters: function(t: table[addr] of dns_summary, v: addr):interval ;  
	global heavy_hitters: table[addr] of dns_summary &create_expire = 1 day &expire_func=expire_heavy_hitters ; 
...
event DNS::log_dns(rec: DNS::Info)&priority=-15 
{
	...
	local query = rec?$query ? rec$query: "Unknown" ; 
        ...
        @if ( Cluster::is_enabled())
                Cluster::publish_hrw(Cluster::proxy_pool, src, DNS::summary, src, resp, query, qtype_name, rcode_name) ;
        @else
...
event DNS::summary(src: addr, resp: addr, query: string, qtype_name: string, rcode_name: string)&priority=-10
{ 
	...
	{ 
		msg = fmt ("%s", print_heavy_hitters(src)) ; 
                NOTICE([$note=DNS::HostThreshold, $src=src, $msg=msg, $identifier=cat(src), $suppress_for=1 min]);
	} 
	...
	if (check_thresh)
		...
		msg = print_heavy_hitters(src); 
                NOTICE([$note=DNS::QueryThreshold, $src=src, $msg=msg, $identifier=cat(src), $suppress_for=1 min]);
        }
...
{
	local qcount = double_to_count(hll_cardinality_estimate(heavy_hitters[src]$query)) ; 
	local rcount = double_to_count(hll_cardinality_estimate(heavy_hitters[src]$hosts)) ; 
	local qtype="" ; 
	...
	local msg = fmt ("IP[%s], numQueries: %s, uniqQueries: %s, Hosts: %s, Qtype: %s", src,  heavy_hitters[src]$num_query, qcount, rcount,  qtype ); 
