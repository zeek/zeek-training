...
global flush_dns_spike_ip_counter:function (t: table[addr] of DNS_SpikeThresholdRecord, orig: addr): interval; 
	...
	global dns_spike_idx: table[addr] of count
                        &default=0 &create_expire = 1 min &redef;
...
#				print fmt ("IP: [%s] ==>  %s", request_ip, dns_spike_ip_counter[request_ip]); 
					...
					local msg = fmt("IP[%s] has done %s (First:%s, cur: %s, diff: %s) look ups: %d", request_ip, dns_spike_ip_counter[request_ip]$tally, dns_spike_ip_counter[request_ip]$first_seen_time, dns_spike_ip_counter[request_ip]$cur_time, dns_spike_ip_counter[request_ip]$cur_time - dns_spike_ip_counter[request_ip]$first_seen_time, n); 
					...
					{
						 if (time_diff < 60 sec ) {
							NOTICE([$note=DNS::DNS_Spike_Threshold, $msg=msg, $identifier=cat(request_ip)]);
...
#				print fmt (" DIFF: request_ip[%s] - %s, count: %d", request_ip, dns_spike_ip_counter[request_ip]$cur_time - dns_spike_ip_counter[request_ip]$first_seen_time, dns_spike_ip_counter[request_ip]$tally );
	...
	# look for thresholds over the day duration  
			...
			NOTICE([$note=DNS::DNS_Threshold, $msg=msg, $identifier=cat(request_ip), $suppress_for=1 min]);
                }
