...
{
	log_reporter(fmt("EVENT: SMTPurl::m_w_add_url_to_bloom: added to bloomfilter - link: %s", link), 10);
...
{
	log_reporter(fmt("EVENT: SMTPurl::w_m_url_click_in_bloom : link: %s", link), 10);
...
{
	log_reporter(fmt("EVENT: function SMTPurl::mail_links_expire_func [ worker ] : link: %s", link), 10);
	bloomfilter_add(mail_links_bloom, link);
	return 0 secs;
}
@endif
...
function mail_links_expire_func(t: table[string] of mi, link: string): interval
{
	log_reporter(fmt("EVENT: function SMTPurl::mail_links_expire_func [ manager ] : link: %s", link), 10);
...
@ifdef ( SMTPurl::sql_write_mail_links_db )
	if ( SMTPurl::sql_write_mail_links_db(link, t[link]) ) {
		bloomfilter_add(mail_links_bloom, link);
		@if ( Cluster::is_enabled() && Cluster::local_node_type() == Cluster::MANAGER )
		Broker::publish(Cluster::worker_topic, SMTPurl::m_w_add_url_to_bloom, link);
		@else
		event SMTPurl::m_w_add_url_to_bloom(link);
		@endif
		return 0 secs;
	} else {
		log_reporter(fmt("Failure in writing to DATABASE so keeping in the mail_links table itself : link: %s, t[link]:%s", link,
		    t[link]), 0);
...
@if ( Cluster::is_enabled() && Cluster::local_node_type() != Cluster::MANAGER )
		Broker::publish(Cluster::manager_topic, SMTPurl::w_m_smtpurls_new, link, mail_info);
@else
...
event SMTPurl::w_m_smtpurls_new(link: string, mail_info: mi)
{
	...
	### send to all workers
	@if ( Cluster::is_enabled() && Cluster::local_node_type() == Cluster::MANAGER )
	Broker::publish(Cluster::worker_topic, SMTPurl::m_w_smtpurls_add, link, mail_info);
	@else
...
{
	log_reporter(fmt("EVENT: SMTPurl::m_w_smtpurls_add: link: %s, mail_info: %s", link,
	    mail_info), 10);
	...
	if ( link !in mail_links ) {
		mail_links[link] = mail_info;
		log_reporter(fmt("EVENT: SMTPurl::m_w_smtpurls_add: link: %s, mail_links[link]: %s", link,
		    mail_links[link]), 0);
	}
}
@endif
